...
# converts dates to a grid of freq points per year starting from the first day of a new year
...
#### ALL run on Cluster now!!! Transfer ... ####
# outlier removal (set to default=F)
outlier = TRUE
yr = 2001 # Split up processing 2001 - 2013
# Ideal time series
...
sites <- subset(sites,startyear==yr)
#### NEED to correct the 25 sites with encoding error in name manually later!!! ###
#sites$SSBS <- iconv(sites$SSBS,"UTF-8","latin1") # Convert encoding
myLog("Loading Files")
...
# Subset only to relevant studies
b1 <- subset(b1,SSBS %in% sites$SSBS)
...
for(id in unique(sites$SSBS)){
  ...
  result[[id]][["EVI2"]] = 2.5 * ((B2 - B1) / (B2 + 2.4 * B1 + 1))
  # SAVI
...
print("Done!")
saveRDS(result,paste0("Center_FullTimeSeriesSmooth_Final_",yr,".rds"))
